    ...
    <span className="relative flex shrink-0" style={{ width: dim, height: dim }}>
      {alive && (
        <span
          ...
          style={{ background: col, animationDuration: "1.5s" }}
        />
      )}
...
function ServiceCard({ svc, health }) {
  ...
  return (
    ...
    >
      {/* hover glow */}
      <div
        className="pointer-events-none absolute -left-4 -top-4 h-24 w-24 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl"
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="shrink-0 text-right">
          {alive === undefined
            ? <span className="text-xs text-muted/50">checking…</span>
            : alive
              ? <span className="text-[11px] font-mono font-bold text-emerald-400">{latency}ms</span>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        >
          {alive === undefined ? "—" : alive ? "online" : "down"}
        </span>
        {checkedAt && (
          <span className="text-[10px] text-muted/60 font-mono">{checkedAt}</span>
        )}
      </div>
    </div>
  );
...
function InfraCard({ item }) {
  ...
  return (
    <div
      ...
      style={{ boxShadow: `0 0 16px -8px ${item.color}40` }}
    >
      <div
        ...
        <div
          ...
          style={{
            ...
            backgroundImage: dashed
              ? `repeating-linear-gradient(90deg, ${color}80 0, ${color}80 5px, transparent 5px, transparent 9px)`
              : undefined,
          }}
        />
...
function VArrow({ label, color = "#52525b", dashed = false }) {
  return (
    <div className="flex flex-col items-center justify-center gap-0.5" style={{ minHeight: 36 }}>
      <div
        className="w-px flex-1"
        style={{
          background: dashed ? "transparent" : color + "80",
          backgroundImage: dashed
            ? `repeating-linear-gradient(180deg, ${color}80 0, ${color}80 5px, transparent 5px, transparent 9px)`
            : undefined,
          minHeight: 20,
        }}
      />
      <div style={{ width: 0, height: 0, borderLeft: "4px solid transparent", borderRight: "4px solid transparent", borderTop: `6px solid ${color}CC` }} />
      {label && (
        <span className="text-[8.5px] font-mono font-semibold px-1 py-px rounded whitespace-nowrap -mt-0.5" style={{ color, background: color + "18" }}>
          {label}
        </span>
      )}
    </div>
  ...
  return (
    <div className="overflow-x-auto">
      <div style={{ minWidth: 620 }} className="px-4 py-6 font-sans">
          ...
          <div className="flex flex-col items-center" style={{ marginRight: 16 }}>
            ...
            <div className="flex items-start gap-0" style={{ width: 320, marginLeft: -90 }}>
              <div className="flex flex-col items-center" style={{ width: 106 }}>
                ...
                <Node label="Redis Cache" sub="hot cache" color="#f43f5e" w={130} />
              </div>
              <div className="flex flex-col items-center" style={{ width: 106 }}>
        ...
        {/* Row 4: RabbitMQ → Worker → Mongo Primary */}
        ...
        <div className="mt-6 flex flex-wrap gap-4 text-[10px] font-mono text-muted">
          {[
            { color: "#6366f1", label: "HTTP proxy",       dashed: false },
            { color: "#fb923c", label: "RabbitMQ publish", dashed: false },
            { color: "#f43f5e", label: "Cache read",       dashed: false },
            { color: "#4ade80", label: "DB fallback",      dashed: true  },
            { color: "#fb923c", label: "link.* (event)",   dashed: true  },
          ].map(({ color, label, dashed }) => (
            <span key={label} className="flex items-center gap-1.5">
              <span style={{
                display: "inline-block", width: 24, height: 1.5,
                background: dashed ? "transparent" : color + "CC",
                backgroundImage: dashed
                  ? `repeating-linear-gradient(90deg,${color}CC 0,${color}CC 5px,transparent 5px,transparent 9px)`
                  : undefined,
              }} />
...
export function ArchitecturePage() {
  ...
  useInterval(checkHealth, 20000); // real ping every 20 s
  ...
  const allHealthy = aliveCount === totalCount && totalCount > 0;
  const gatewayRTT = health["gateway"]?.latency;
  const hasAnyData = Object.keys(health).length > 0;
  ...
  return (
    <div className="grid gap-6 animate-page-in">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-full border border-white/[0.06] bg-white/[0.03] px-3 py-1.5">
            <PulseDot alive={hasAnyData ? allHealthy : undefined} size="sm" />
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex flex-col items-center gap-1 rounded-xl border border-white/[0.06] bg-white/[0.03] px-5 py-3.5 text-center min-w-[110px]">
          <span
            className="text-2xl font-bold font-mono leading-none"
            style={{ color: allHealthy ? "#10b981" : "#f59e0b" }}
          >
            {hasAnyData ? `${aliveCount}/${totalCount}` : "—"}
          </span>
          <span className="text-[11px] text-muted font-medium uppercase tracking-widest leading-tight">Services online</span>
        </div>
        {gatewayRTT != null && (
          <div className="flex flex-col items-center gap-1 rounded-xl border border-white/[0.06] bg-white/[0.03] px-5 py-3.5 text-center min-w-[110px]">
            <span className="text-2xl font-bold font-mono leading-none" style={{ color: "#6366f1" }}>
              {gatewayRTT}ms
            </span>
            <span className="text-[11px] text-muted font-medium uppercase tracking-widest leading-tight">Gateway RTT</span>
          </div>
        )}
        <div className="flex flex-col items-center gap-1 rounded-xl border border-white/[0.06] bg-white/[0.03] px-5 py-3.5 text-center min-w-[110px]">
          <span className="text-2xl font-bold font-mono leading-none" style={{ color: "#8b5cf6" }}>
            {SERVICES.length}
          </span>
          <span className="text-[11px] text-muted font-medium uppercase tracking-widest leading-tight">Microservices</span>
        </div>
        <div className="flex flex-col items-center gap-1 rounded-xl border border-white/[0.06] bg-white/[0.03] px-5 py-3.5 text-center min-w-[110px]">
          <span className="text-2xl font-bold font-mono leading-none" style={{ color: "#22c55e" }}>
            {INFRA.length}
          </span>
          <span className="text-[11px] text-muted font-medium uppercase tracking-widest leading-tight">Infra nodes</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {SERVICES.map((svc) => (
            <ServiceCard key={svc.id} svc={svc} health={health[svc.id]} />
          ))}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
        ...
        <p className="mb-4 text-xs text-muted leading-relaxed max-w-3xl">
          Every redirect checks <span className="text-red-400 font-mono">Redis</span> first (
          <code className="text-[10px] bg-white/5 px-1 rounded">redirect:&#123;code&#125;</code>).
          ...
          The <span className="text-pink-400 font-mono">Analytics Worker</span> consumes, parses UA + GeoIP, and upserts to <span className="text-emerald-300 font-mono">Mongo Primary</span>.
        </p>
        <FlowDiagram />
      </div>

      {/* ── Scalability Notes + Dependency Map ── */}
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
          ...
          <div className="grid gap-2 text-[11px] font-mono">
            {[
              ...
              { from: "Auth Service :3001",     deps: ["MongoDB Primary (write)", "RabbitMQ (email events)"] },
              { from: "Link Service :3002",     deps: ["MongoDB Primary (write)", "RabbitMQ → link.created / link.updated / link.deleted"] },
              { from: "Redirect Service :3003", deps: ["Redis GET redirect:{code}", "MongoDB Replica (miss fallback)", "RabbitMQ publish → analytics.clicks", "RabbitMQ subscribe → link.*"] },
              { from: "Analytics Worker",       deps: ["RabbitMQ consume analytics.clicks", "MongoDB Primary → Click + Analytics upsert"] },
              { from: "Analytics API :3005",    deps: ["MongoDB Replica (secondaryPreferred)"] },
              { from: "Notification Service",   deps: ["RabbitMQ consume email_queue", "SMTP (Gmail)"] },
            ].map(({ from, deps }) => (
              <div key={from} className="rounded-xl border border-white/[0.05] bg-white/[0.02] p-3">
                <p className="font-bold text-ink mb-1.5 text-[11px]">{from}</p>
